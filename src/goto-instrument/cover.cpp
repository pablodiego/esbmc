...
static void instrument_cover_goals(
    const irep_idt &function_id, goto_programt &goto_program,
    const cover_instrumenterst &instrumenters, const irep_idt &mode,
  ...
  basic_blocks->report_block_anomalies(function_id, goto_program,
                                       message_handler);
...
void cover_instrumenterst::add_from_criterion(
    coverage_criteriont criterion, const symbol_tablet &symbol_table,
    const goal_filterst &goal_filters) {
  switch (criterion) {
  case coverage_criteriont::LOCATION:
    instrumenters.push_back(util_make_unique<cover_location_instrumentert>(
        symbol_table, goal_filters));
    break;
  case coverage_criteriont::BRANCH:
    instrumenters.push_back(util_make_unique<cover_branch_instrumentert>(
        symbol_table, goal_filters));
    break;
  case coverage_criteriont::DECISION:
    instrumenters.push_back(util_make_unique<cover_decision_instrumentert>(
        symbol_table, goal_filters));
    break;
  case coverage_criteriont::CONDITION:
    instrumenters.push_back(util_make_unique<cover_condition_instrumentert>(
        symbol_table, goal_filters));
    break;
  ...
  case coverage_criteriont::ASSERTION:
    instrumenters.push_back(util_make_unique<cover_assertion_instrumentert>(
        symbol_table, goal_filters));
    break;
  case coverage_criteriont::COVER:
    instrumenters.push_back(util_make_unique<cover_cover_instrumentert>(
        symbol_table, goal_filters));
  }
}
...
void parse_cover_options(const cmdlinet &cmdline, optionst &options) {
  ...
  options.set_option("cover-include-pattern",
  ...
  options.set_option("cover-traces-must-terminate",
...
static void instrument_cover_goals(const cover_configt &cover_config,
  ...
  if (cover_config.function_filters(function_symbol, function)) {
    ...
    instrument_cover_goals(
        function_symbol.name, function.body, cover_config.cover_instrumenters,
        function_symbol.mode, message_handler, cover_config.make_assertion);
    changed = true;
  }

  if (cover_config.traces_must_terminate &&
...
void instrument_cover_goals(const cover_configt &cover_config,
  ...
  instrument_cover_goals(cover_config, function_symbol,
                         function.get_goto_function(), message_handler);
...
bool instrument_cover_goals(const cover_configt &cover_config,
  ...
  Forall_goto_functions(f_it, goto_functions) {
    ...
    instrument_cover_goals(cover_config, function_symbol, f_it->second,
                           message_handler);
  }
...
bool instrument_cover_goals(const cover_configt &cover_config,
                            ...
                            message_handlert &message_handler) {
  return instrument_cover_goals(cover_config, goto_model.symbol_table,
