  ...
  forall_goto_program_instructions(it, _goto_program) {
    ...
    if (next_is_target || it->is_target()) {
      if (auto block_number = continuation_of_block(it, block_map)) {
        ...
        block_infos.back().source_location = locationt ::nil();
        current_block = block_infos.size() - 1;
      }
    }

    INVARIANT(current_block < block_infos.size(),
...
std::size_t
cover_basic_blockst::block_of(goto_programt::const_targett t) const {
...
void cover_basic_blockst::report_block_anomalies(
    const irep_idt &function_id, const goto_programt &goto_program,
